class OrderRecordsController < ApplicationController
  ...
  def item_set
    ...
    redirect_to root_path if current_user.id == @item.user.id || @item.order_record.present?
  end

  def order_params
    params.require(:order_record_payment).permit(:postcode, :delivery_source_id, :city, :block, :building, :phone_number).merge(
